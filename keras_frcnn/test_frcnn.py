...
parser.add_option("--backbone_network", dest="backbone_network", help="Base network including vgg and resnet50.",
                  default='resnet50')
parser.add_option("--config_filename", dest="config_filename",
                  help="Location to read the metadata related to the training (generated when training).",
                  default="config.pickle")
parser.add_option("--input_weight_path", dest="input_weight_path", help="Input path for trained weights.",
                  default='./model_frcnn.hdf5')
parser.add_option("--is_show_cropped", dest="is_show_cropped", help="Whether plot the cropped images. (Default=false)",
                  default=False)
parser.add_option("--is_save_cropped", dest="is_save_cropped", help="Whether save the cropped images. (Default=false)",
                  default=False)
parser.add_option("--is_show_whole", dest="is_show_whole",
                  help="Whether plot the whole images with predicted bounding box. (Default=false)", default=False)
parser.add_option("--is_save_whole_image", dest="is_save_whole_image",
                  help="Whether save the whole images with predicted bounding box. (Default=false)", default=False)
parser.add_option("--save_img_root_folder_path", dest="save_img_root_folder_path",
                  help="Folder path of storing predicted images. ")
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for key in bboxes:
        ...
        max_idx = list(new_probs).index(max(new_probs))
            ...
            if jk == max_idx:
                ...
                if options.is_save_cropped:
                    ...
                    if new_boxes.shape[0] == 1:
                        ...
                        tongue_cropped_whole_path = os.path.join(save_path,
                                                                 img_name.split('.')[0] + '_whole_' + str(jk + 1) + '.jpg')
                        shebian_1_path = os.path.join(save_path,
                                                      img_name.split('.')[0] + '_shebian_1_' + str(jk + 1) + '.jpg')
                        shebian_2_path = os.path.join(save_path,
                                                      img_name.split('.')[0] + '_shebian_2_' + str(jk + 1) + '.jpg')
